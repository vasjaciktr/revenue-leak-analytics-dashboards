...
WITH base AS (
  ...
  FROM `T.YOUR_GA4_DATASET.events_*`
  WHERE _TABLE_SUFFIX BETWEEN start_date AND end_date
),
...
CREATE OR REPLACE TABLE `T.leakonic.funnel_transitions_by_device` AS
...
CREATE OR REPLACE TABLE `T.leakonic.device_funnel_priority` AS
...
WITH device_leaks AS (
  ...
  FROM `T.leakonic.funnel_transitions_by_device`
  GROUP BY device_category
),
